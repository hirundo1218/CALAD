...
from utils.common_config import get_train_transformations, get_val_transformations1, get_train_dataset, get_train_dataloader, get_aug_train_dataset, get_val_dataset, get_val_dataloader, get_optimizer, get_model, get_criterion, adjust_learning_rate, inject_sub_anomaly
...
def main():
    ...
    for epoch in range(start_epoch, p['epochs']):
        ...
        self_sup_classification_train(train_dataloader, model, criterion, optimizer, epoch, p['update_cluster_head_only'])
        if (epoch == p['epochs']-1):
        ...
        if rep_f1 > best_f1:
            ...
            torch.save({'optimizer': optimizer.state_dict(), 'model': model.state_dict(), 'epoch': epoch + 1, 'best_loss': best_loss, 'best_loss_head': best_loss_head, 'normal_label': normal_label}, p['classification_checkpoint'])


    model_checkpoint = torch.load(p['classification_model'], map_location='cpu')
    model.module.load_state_dict(model_checkpoint['model'])
    torch.save({'optimizer': optimizer.state_dict(), 'model': model.state_dict(), 'epoch': p['epochs'], 'best_loss': best_loss, 'best_loss_head': best_loss_head, 'normal_label': normal_label}, p['classification_checkpoint'])
